    ...
    override suspend fun getReminder(id: String): com.udacity.project4.locationreminders.data.dto.Result<ReminderDTO> = withContext(ioDispatcher) {
        wrapEspressoIdlingResource {
            try {
                ...
                if (reminder != null) {
                    return@withContext com.udacity.project4.locationreminders.data.dto.Result.Success(
                        reminder
                    )
                } else {
                    return@withContext com.udacity.project4.locationreminders.data.dto.Result.Error(
                        "Reminder not found!"
                    )
                }
            } catch (e: Exception) {
